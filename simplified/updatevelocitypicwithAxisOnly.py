...
input = surface_data
input_temp =   (input[1,:,:]**2+input[2,:,:]**2)**0.5
x1 = range(0,101,20)
y1 = range(0,81,20)
xx1 = []
xx2 = []
...
plt.figure(dpi=600)
plt.imshow(input_temp[290:370, 550:650], cmap='jet')
hhhh = plt.colorbar(orientation = 'vertical')
plt.xticks(x1,xx1[0:6])
plt.yticks(y1,xx2[0:5])
hhhh.set_label('unit(m/s)',fontsize=10,weight='bold')
...
# input = np.load(os.path.join('.\input_data\store\input_surface.npy')).astype(np.float32)
input1_temp =   (input1[1,:,:]**2+input1[2,:,:]**2)**0.5
plt.figure(figsize=(5,3))
plt.rc('font',family='Times New Roman',size=10,weight='bold')
plt.figure(dpi=600)
plt.imshow(input1_temp[290:370, 550:650], cmap='jet')
hhhh = plt.colorbar(orientation = 'vertical')
plt.xticks(x1,xx1[0:6])
plt.yticks(y1,xx2[0:5])
hhhh.set_label('unit(m/s)',fontsize=10,weight='bold')
...
data = abs(input_temp-input1_temp)/abs(input1_temp)

plt.figure(figsize=(5,3))
plt.rc('font',family='Times New Roman',size=10,weight='bold')
plt.figure(dpi=600)

plt.imshow(data[290:370, 550:650], cmap='jet')
hhhh = plt.colorbar(orientation = 'vertical')
plt.xticks(x1,xx1[0:6])
plt.yticks(y1,xx2[0:5])
